...
void print_bytes(unsigned char *buffer, size_t index, size_t count) {
...
size_t disassemble_instruction(unsigned char *buffer, size_t index) {
    unsigned char opcode = buffer[index];
    unsigned char arg1 = buffer[index + 1];
    unsigned char arg2 = buffer[index + 2];
    unsigned char arg3 = buffer[index + 3];
    switch(opcode) {
        case 0x00:
            PRINT(1); printf("NOP\n"); return 1;
        case 0x06:
